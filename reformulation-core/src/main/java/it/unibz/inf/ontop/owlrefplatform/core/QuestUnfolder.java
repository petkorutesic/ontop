...
*/
    ...
    private void preprocessUnlabeledBlankNodeTemplates(Collection<OBDAMappingAxiom> mappings, DBMetadata metadata) {
        ...
        for (OBDAMappingAxiom mapping : mappings) {
            try {
                ...
                Map<Term, Term> map = getBNodeTemplateReplacementMap(metadata, mapping, select, targetQuery, dfac);
    ...
    private Map<Term, Term> getBNodeTemplateReplacementMap(DBMetadata metadata, OBDAMappingAxiom mapping, Select select, List<Function> targetQuery, OBDADataFactory dfac) {
