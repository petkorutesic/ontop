...
import it.unibz.inf.ontop.owlrefplatform.core.queryevaluation.H2SQLDialectAdapter;
...
public class SQLGenerator implements SQLQueryGenerator {
    ...
    private List<String> rowNumberModifiers = null;
...
//			log.debug("Normalized CQ: \n{}", cq);
    ...
    public String getSQLString(Term term, QueryAliasIndex index, boolean useBrackets) {
        ...
        if (function.isDataTypeFunction()) {
            ...
            if (functionSymbol == ExpressionOperation.REGEX) {
                ...
                if (sqladapter instanceof H2SQLDialectAdapter) {
					/*
					 * A workaround to handle H2 database problem of not having ability to add order by in OVER clause
					 * so instead of the usual way: SELECT ROW_NUMBER() OVER (ORDER by attr1, attr2)
					 * we have to insert order by attributes in the standard place for ORDER BY clauses of the main SELECT query
					 * http://stackoverflow.com/questions/32360807/row-number-over-with-order-by-in-h2/32368073#32368073
					 */
                    if (rowNumberModifiers == null && !function.getTerms().isEmpty()) {
                        rowNumberModifiers = orderbyList;
                    }
                }
    ...
    public class QueryAliasIndex {
         ...
         * boolean conditions.
         *
         * @param var The variable we want the referenced columns.
         */
